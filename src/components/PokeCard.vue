<template>
 <div class="max-w-xs sm:max-w-xs md:max-w-xs lg:max-w-xs xl:max-w-xs shadow-lg rounded-lg overflow-hidden bg-bottom bg-cover" :style="{ backgroundImage: `url(${backgroundType})`,borderColor: typeColor,borderStyle: 'solid', borderWidth: '1px'}" style="height: 300px; max-width: 250px;">
  <div class="w-full flex justify-center">
    <img :src="pokemonSprite" alt="pokeimg" class="w-60 transform translate-y-5">
  </div>
  <div :style="{ borderColor: typeColor, backgroundColor: backgroundColor }" class="w-full h-full border-t-2 rounded-tr-2xl text-gray-800 bg-opacity-50">
    <div class="flex justify-between mb-4">
      <div :style="{ background: typeColor }" :class="textColorClass" class="rounded-r-3xl p-1 pr-5 pl-5 text-white font-bold z-10">
        <p>{{ capitalizedPokemonName }}</p>
      </div>
      <span :class="textColorClass" class="p-2 pr-4 pl-3 text-sm">N.º{{ pokemonId }}</span>
    </div>
...
const typeColors = {
  ...
  dragon: '#8A2BE2',   // Azul violeta
  ice: '#00BFFF',      // Azul claro
  steel: '#A9A9A9',    // Gris
  rock: '#A52A2A',     // Marrón rojizo
  grass: '#32CD32',    // Verde
  ground: '#8B4513',   // Marrón
  poison: '#800080',   // Púrpura oscuro
  fighting: '#DC143C',    // Rojo oscuro
  ghost: '#663399',    // Morado oscuro
  bug: '#A8B820',
  normal:'#A8A878'
